...
public class ring {
	...
	public static void main(String[] args) {
		...
		while (elapsedSinceStart() < LEAVETIME) {
			...
			if (elapsedSinceStart() < LEAVETIME) {
	...
	public static void startElection() {
		...
		writeToLog(String.format("started election, send election message to client %d", NEXT), new Throwable().getStackTrace()[0].getMethodName());
	}
	...
	public static void handleElection(Election election) {
		...
		if (MYPORT == election.bestSoFar) {
			...
			writeToLog(String.format("leader selected", election.electionID), new Throwable().getStackTrace()[0].getMethodName());
			return;
		}
	...
	public static void writeToLog(String msg, String method) {
		...
		int minutes = time / 60;
		int seconds = time % 60;
		String output = String.format("%d:%02d: %s\n", minutes, seconds, msg);
		WRITER.write(output);
		WRITER.flush();
